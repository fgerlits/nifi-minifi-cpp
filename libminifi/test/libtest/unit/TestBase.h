...
class TypedProcessorWrapper {
 public:
  ...
  // it's a wrapper over the same object, with no value change
...
class TestPlan {
 public:
  ...
  minifi::core::Processor* addProcessor(const std::string &processor_name, const minifi::utils::Identifier& uuid, const std::string &name,
    const std::initializer_list<minifi::core::Relationship>& relationships, bool linkToPrevious = false);
...
static std::unique_ptr<core::extension::ExtensionManager> extensionInitializer = [] {
...
#endif
  return std::make_unique<core::extension::ExtensionManager>(config);
}();
#endif
