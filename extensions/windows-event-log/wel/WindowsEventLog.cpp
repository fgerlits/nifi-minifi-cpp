...
std::optional<std::string> EventDataCache::get(EVT_FORMAT_MESSAGE_FLAGS field, const std::string& value) {
    std::lock_guard<std::mutex> lock{mutex_};
    const auto it = cache_.find(std::make_tuple(field, value));
    if (it != cache_.end() && it->second.expiry > std::chrono::system_clock::now()) { return it->second.value; }
    return std::nullopt;
}

void EventDataCache::set(EVT_FORMAT_MESSAGE_FLAGS field, const std::string& old_value, std::string new_value) {
  std::lock_guard<std::mutex> lock{mutex_};
  cache_.insert_or_assign(std::make_tuple(field, old_value), CacheItem{std::move(new_value), std::chrono::system_clock::now() + lifetime_});
}
...
std::string WindowsEventLogMetadataImpl::getEventData(EVT_FORMAT_MESSAGE_FLAGS field, const std::string& value) const {
  return metadata_ptr_.getEventData(field, value, event_ptr_);
}

std::string WindowsEventLogHandler::getEventData(EVT_FORMAT_MESSAGE_FLAGS field, const std::string& value, EVT_HANDLE event_ptr) const {
  auto cached_value = event_data_cache_.get(field, value);
  if (cached_value) { return *cached_value; }
  auto new_value = getEventDataImpl(field, value, event_ptr);
  event_data_cache_.set(field, value, new_value);
  return new_value;
}

std::string WindowsEventLogHandler::getEventDataImpl(EVT_FORMAT_MESSAGE_FLAGS field, const std::string& value, EVT_HANDLE event_ptr) const {
