...
std::string updateXmlMetadata(const std::string &xml, EVT_HANDLE metadata_ptr, EVT_HANDLE event_ptr, bool update_xml, bool resolve, utils::Regex const* regex = nullptr) {
  ...
  MetadataWalker walker(metadata, "", update_xml, resolve, regex, [](const std::string& id) { return utils::OsUtils::userIdToUsername(id, nullptr); });
...
void extractMappingsTestHelper(const std::string &file_name,
  ...
  auto regex = utils::Regex(".*Sid");
  MetadataWalker walker(FakeWindowsEventLogMetadata{}, METADATA_WALKER_TESTS_LOG_NAME, update_xml, resolve, &regex, [](const std::string& id) {
    return utils::OsUtils::userIdToUsername(id, nullptr);
  });
  doc.traverse(walker);
