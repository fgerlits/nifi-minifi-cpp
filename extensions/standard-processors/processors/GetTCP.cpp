...
void GetTCP::onSchedule(core::ProcessContext& context, core::ProcessSessionFactory&) {
  ...
  auto ssl_context = [&]() -> std::optional<asio::ssl::context> {
    if (auto ssl_context_service = utils::parseOptionalControllerService<minifi::controllers::SSLContextService>(context, SSLContextService, getUUID()); ssl_context_service) {
